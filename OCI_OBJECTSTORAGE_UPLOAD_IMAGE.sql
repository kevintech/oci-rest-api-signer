...
create or replace function UPLOAD_IMAGE( p_identity_domain_name in varchar2,
  p_oci_namespace in varchar2,
  p_oci_bucket_name in varchar2,
  p_oci_file_name in varchar2,
  p_oci_file_data in blob ) 
return CLOB is
  ...
  g_OCI_API_VERSION VARCHAR2(16):= '';                 -- Oracle Cloud Infrastructure API version
...
begin
  ...
  l_service_uri := g_OCI_API_VERSION || '/n/' || p_oci_namespace || '/b/' || p_oci_bucket_name || '/o/' || p_oci_file_name;
  
  l_users_filter := 'compartmentId=' || replace(l_tenant_ocid,':','%3A');-- || '&' || 'limit=50';
      
  apex_web_service.g_request_headers(3).name := 'Authorization';
  apex_web_service.g_request_headers(3).value := signPutRequest( l_date_header, l_service_uri || '?' || l_users_filter, l_host_header, 'body', l_tenant_ocid, l_administrator_ocid, l_administrator_key_fingerprint, l_administrator_private_key, true );
